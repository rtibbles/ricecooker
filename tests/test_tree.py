...
def test_do_file_upload_requests_resumable(channel):
    """Test that do_file_upload requests a resumable upload session."""
    filename = "test_file.mp4"
    ...
    with (
        ...
        patch("ricecooker.managers.tree.resumable_upload"),
    ):
        manager.do_file_upload(filename)
...
def test_do_file_upload_resumable_calls_client(channel):
    """Test that do_file_upload delegates to resumable_upload on a resumable session."""
    filename = "test_file.mp4"
    manager, file_data = _make_upload_manager(channel, filename)
    ...
    with (
        ...
        patch("ricecooker.config.get_storage_path", return_value="/tmp/test_file.mp4"),
        patch("ricecooker.config.SESSION.post", return_value=mock_response),
        patch("ricecooker.config.SESSION.put") as mock_put,
        patch("ricecooker.managers.tree.resumable_upload") as mock_resumable,
    ):
        manager.do_file_upload(filename)

    mock_resumable.assert_called_once()
    args = mock_resumable.call_args.args
    assert args[0] is config.SESSION
    assert args[1] == "https://storage.googleapis.com/sess"
    assert args[2] is mocked_open.return_value
    assert args[3] == file_data.size
    mock_put.assert_not_called()


def test_do_file_upload_resumable_already_uploaded_skips(channel):
    """Test that do_file_upload returns without calling resumable_upload when already uploaded."""
    filename = "test_file.mp4"
    manager, file_data = _make_upload_manager(channel, filename)
