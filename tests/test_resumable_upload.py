...
from ricecooker.utils.resumable_upload import CHUNK_SIZE
...
class FakeSession:
    """Records `put` calls and returns canned statuses/exceptions, one per call."""

    def __init__(self, statuses):
        self.statuses = list(statuses)
        self.calls = []

    def put(self, url, data=None, headers=None, allow_redirects=None):
        ...
        if isinstance(status, tuple):
            status_code, headers_out = status
...
def test_gcs_retry_adapter_mounted():
    from ricecooker import config

    adapter = config.SESSION.get_adapter("https://storage.googleapis.com/x")
    assert 503 in adapter.max_retries.status_forcelist

    studio_adapter = config.SESSION.get_adapter(
        "https://api.studio.learningequality.org/x"
    )
    assert (
        getattr(studio_adapter.max_retries, "status_forcelist", None)
